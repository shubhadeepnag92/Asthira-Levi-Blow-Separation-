...
!	MODIFICATION IN v25
	...
	real*8 :: pe1(-5000:5000),pe2(-5000:50000),E_g11,E_g12,E_g22,E_g1h,E_g2h,guestguest_E,guesthost_E
	...
	real*8 :: c0(1:2),c1(1:2),c2(1:2),c3(1:2),initial_rg_atomic(1:3,1:10,1:1000)
	...
	real*8 :: initial_rg_neo(1:3,1:5), initial_rg_npen(1:3,1:5), yt
	...
	integer :: initial_neotype(1:5), initial_npentype(1:5),kgmol,neo_number,npen_number
...
!	OPEN FILES TO READ AND  WRITE
	...
	open(unit=22,file='atomic_neopentane.xyz')
	open(unit=23,file='atomic_n-pentane.xyz')
	...
	do igatom = 1,ngatom
		...
		read(22,*) initial_neotype(igatom),initial_rg_neo(:,igatom)
	...
	do igatom = 1,ngatom
		...
		read(23,*) initial_npentype(igatom),initial_rg_npen(:,igatom)
...
1	format(20x,A)
	write(10,*) 
	if(simulation.eq.0) write(10,2) '$********************HOMOGENEOUS   SIMULATION********************$'
2	format(40x,A)
	if(simulation.eq.1) write(10,3) '$*****INHOMOGENEOUS SIMULATION (with hot-zone perpendicular to cage-window distance)****$'
3	format(20x,A)
	if(simulation.eq.2) write(10,4) '$*****INHOMOGENEOUS SIMULATION (with hot-zone perpendicular to X-axis)*******$'
4	format(10x,A)
	write(10,*)
...
44      format(A,10x,A,10x,A,10x,A)
	...
	neo_number = 0; npen_number = 0
	do igmol = 1,ngmol
		if(gmoltype(igmol).eq.1) 	neo_number = neo_number + 1
		if(gmoltype(igmol).eq.2) 	npen_number = npen_number + 1
	enddo !igmol
...
!	PERFORM CREATTION OR DELETION OF HYDROCARBONS

	!	choose a random number
	
		call random_number(xrand)
		if(xrand.ge.0.5) then
		...
		!	create the hydrocarbon

			ngmol = ngmol + 1
		
			call random_number(zrand)
			com(1,ngmol) = (0.333*zrand+1)*74.5608
			!write(60,*) zrand,com(1,ngmol)
			call random_number(zrand)
			com(2,ngmol) = (0.4*zrand+1)*24.8536
			!write(60,*) zrand,com(2,ngmol)
			call random_number(zrand)
			com(3,igmol) = zrand*boxlengthz
			!write(60,*) zrand,com(3,ngmol)

			!	choose another random number

				call random_number(yrand)
				if(yrand.ge.0.5) then
				...
				!	create neopentane
					...
					do igatom=1,ngatom
						rg_atomic(:,igatom,ngmol) = com(:,ngmol) + initial_rg_neo(:,igatom)
						gatomtype(igatom,ngmol) = initial_neotype(igatom)
					enddo !igatom

					write(60,*) imc,ngmol,gmoltype(ngmol)
			...
			!	create n-pentane
					...
					do igatom=1,ngatom
						rg_atomic(:,igatom,ngmol) = com(:,ngmol) + initial_rg_npen(:,igatom)
						gatomtype(igatom,ngmol) = initial_npentype(igatom)
					enddo !igatom
				...
				!write(60,*) yrand,gmoltype(ngmol),dhtype(ngmol)
				!write(60,*) com(:,ngmol)
				!do igatom = 1,ngatom
				!write(60,*) gatomtype(igatom,ngmol),rg_atomic(:,igatom,ngmol)
				!enddo !igatom
				...
				!	calculation of inter & intra molecular energy
				   ...
				   do igatom=1,ngatom
				      ...
				      do jgatom=1,ngatom
				      	...
				      	E_new = E_new + fourepsgg(igatomtype,jgatomtype) * ( s12 - s6 )
		...
		!	GUEST-HOST
			...
			!	applying minimum boundary condition
					...
					rsq=dx*dx+dy*dy+dz*dz
				      	...
				      	E_new = E_new + fourepsgh(igatomtype,ihatomtype) * ( s12 - s6 )
			...
			dE = dE + R*T*log((ngmol+1)*1.0) - R*T*log(ngmol*1.0)
				
			if (dE.gt.0) then
				call random_number(yrand)
				prob = exp(-dE/(R*T)) 
			endif 
			if (dE.lt.0.or.yrand.lt.prob) then
				ngmol = ngmol	! ACCEPT THE INCREASED NUMBER OF PARTICLES
			else
				ngmol = ngmol-1 ! REJECT THE ADDITON
			endif

			write(60,*) imc,E_old,E_new

		else

		! 	attempt to destroy the hydrocarbon

		!	choose a random number

			call random_number(zrand)

			if(zrand.ge.0.5) then

			! 	attempt to destroy neopentane

			!	choose a random number

			32	call random_number(yrand)

				kgmol = yrand * ngmol

				if (gmoltype(kgmol).ne.1) then

					goto 32

				endif

				if(com(1,kgmol).ge.74.5608d0.and.com(1,kgmol).le.91.4144d0) then
					if(com(2,kgmol).ge.24.8536.and.com(2,kgmol).le.34.8536) then
						goto 35
					else
						goto 32
					endif
			...
			! 	attempt to destroy n-pentane

			!	choose a random number

			33	call random_number(yrand)

				kgmol = yrand * ngmol

				if (gmoltype(kgmol).ne.2) then
				...
				!	calculation of inter & intra molecular energy
				   ...
				   do igatom=1,ngatom
				      ...
				      do jgatom=1,ngatom
				      	...
				      	E_old = E_old + fourepsgg(igatomtype,jgatomtype) * ( s12 - s6 )
		...
		!	GUEST-HOST
			...
			!	applying minimum boundary condition
					...
					rsq=dx*dx+dy*dy+dz*dz
				      	...
				      	E_old = E_old + fourepsgh(igatomtype,ihatomtype) * ( s12 - s6 )
			...
			dE = dE + R*T*log(ngmol*1.0) - R*T*log((ngmol-1)*1.0)
			...
			if (dE.lt.0.or.yrand.lt.prob) then
				ngmol = ngmol-1	! ACCEPT THE INCREASED NUMBER OF PARTICLES
			else
				ngmol = ngmol ! REJECT THE ADDITON
			endif
...
!	Only If you can not keep the coordinates correct after rotation, angular and dihedral motion
	...
	write(60,*) imc,ngmol,neo_number,npen_number
...
!  ---------- DIHEDRAL MOTION ----------	!
	...
	dh_rotation = 0
        ...
        !do i=-5000,5000
        !        count1(i)=0;count2(i)=0.d0;pe1(i)=0.0d0;pe2(i)=0.d0
        !enddo !i
        !endif
	...
	do igmol = 1,ngmol
            ...
            if(gmoltype(igmol).eq.1) then
                ...
                !pt_distance = (-com(1,igmol)-com(2,igmol)+com(3,igmol) ) / 1.732050808
                !pt_distance = sqrt (com(1,igmol)**2+com(2,igmol)**2+com(3,igmol)**2)

                !ibin1 = nint(pt_distance/0.1d0) 		!	if you want to bin perp distance from nearest window plane to com of a molecule
                !if(pt_distance.ge.-0.05.and.pt_distance.le.0.05) write(60,*) igmol,pt_distance,ibin1,E        
                ...
                count1(ibin1)=count1(ibin1)+1
                pe1(ibin1) = pe1(ibin1) + E
                !write(30,*)
                !pt_distance,ibin1!,count1(ibin1),pe1(ibin1),E
                !if(ibin1.gt.55) stop
           endif

           if(gmoltype(igmol).eq.2) then
		call energyith                   ! calculate energy of igmol
                call cagetocomdistance           ! calculate prep distance from nearest window to com of igmol
                !pt_distance = (-com(1,igmol)-com(2,igmol)+com(3,igmol) ) / 1.732050808
                !pt_distance = sqrt (com(1,igmol)**2+com(2,igmol)**2+com(3,igmol)**2)

                !ibin2 = nint(pt_distance/0.1d0)	!	if you want to bin perp distance from nearest window plane to com of a molecule
                ibin2 = nint(com(1,igmol)/0.1d0)		!	if you want to bin com position of a molecule

                !if(pt_distance.ge.-0.05.and.pt_distance.le.0.05) write(70,*) igmol,pt_distance,ibin2,E
                ...
                count2(ibin2)=count2(ibin2)+1
                pe2(ibin2) = pe2(ibin2) + E
                !write(30,*)
                !pt_distance,ibin1!,count1(ibin1),pe1(ibin1),E
                !if(ibin2.gt.55) stop
           endif

	   if(iwinfo.eq.int(nwinfo-1)) then

!	   	if(gmoltype(igmol).eq.1) write(17,*) 'c',com(:,igmol),guestguest_E,guesthost_E
!	   	if(gmoltype(igmol).eq.2) write(17,*) 'n',com(:,igmol),guestguest_E,guesthost_E
...
50      format(1x,i3,1x,f6.2,1x,f6.2,1x,f6.2)
        enddo !igmol
...
!	WRITE THE TRAJECTORY OF HOST & GUEST MOLECULES
	...
	do igmol=1,ngmol
	   ...
	   do igatom=1,ngatom
	   	if(gmoltype(igmol).eq.1) write(8,*) 'c',rg_atomic(:,igatom,igmol)
		if(gmoltype(igmol).eq.2) write(70,*) 'C',rg_atomic(:,igatom,igmol)
	   enddo !iatom

	   do igatom=1,ngatom
	   	if(gmoltype(igmol).eq.2) write(8,*) 'p',rg_atomic(:,igatom,igmol)
		if(gmoltype(igmol).eq.2) write(70,*) 'p',rg_atomic(:,igatom,igmol)
	   enddo 
...
!	WRITE THE DENSITY PROFILE
	...
	do i = -3000,3000
	  write(40,*) i*0.1d0,pe1(i)/count1(i),count1(i),pe1(i),count1(i)*1.0d0/(100000)
	  write(50,*) i*0.1d0,pe2(i)/count2(i),count2(i),pe2(i),count2(i)*1.0d0/(100000)
	enddo !i
...
!	write(30,*) 'ith',finish-start
	...
	!write(*,*) igmol,com(:,igmol)
	!write(*,*) igmol,com_new1,com_new2,com_new3

	!if(igmol.eq.6) then
	!write(102,*) imc,igmol
	!write(102,*) com(:,igmol)
	!write(102,*) com_new1,com_new2,com_new3
	!endif

	if(com(1,igmol).lt.74.5608d0.or.com(1,igmol).gt.99.4144d0) then
		...
		do while (com_new2.gt.boxlengthy)
            	call random_number(xrand)
        	com_new2=com(2,igmol)+(2*xrand-1)*dr_max
        	end do ! while
		...
		do while (com_new3.gt.boxlengthz)
            	call random_number(xrand)
        	com_new3=com(3,igmol)+(2*xrand-1)*dr_max
        	end do ! while
		...
		!write(*,*) 'let',igmol,com_new1,com_new2,com_new3

	endif

	if(com(1,igmol).ge.74.5608d0.and.com(1,igmol).le.99.4144d0) then
		...
		if(com_new2.ge.24.8537d0) then

			!write(*,*) 'inlet1',igmol,com_new1,com_new2,com_new3

			do while (com_new1.gt.99.4144d0)
            		call random_number(xrand)
        		com_new1=com(1,igmol)+(2*xrand-1)*dr_max
			!write(*,*) 'inlet1x',igmol,com_new1,com_new2,com_new3
        		end do ! while

        		do while (com_new1.lt.74.5608d0)
        		call random_number(xrand)
        		com_new1=com(1,igmol)+(2*xrand-1)*dr_max
			!write(*,*) 'inlet1x',igmol,com_new1,com_new2,com_new3
        		end do ! while
	
			do while (com_new2.gt.34.8536d0)
            		call random_number(yrand)
        		com_new2=com(2,igmol)+(2*yrand-1)*dr_max
			!write(*,*) 'inlet1y',igmol,com_new1,com_new2,com_new3
        		end do ! while
			...
			do while (com_new3.gt.boxlengthz)
            		call random_number(xrand)
        		com_new3=com(3,igmol)+(2*xrand-1)*dr_max
			!write(*,*) 'inlet1z',igmol,com_new1,com_new2,com_new3
        		end do ! while
		...
		if(com_new2.lt.24.8537d0) then

			!write(*,*) 'inlet2',igmol,com_new1,com_new2,com_new3
		...
		!write(*,*) 'inlet',igmol,com_new1,com_new2,com_new3
...
!	for the case when the x-component is less than 0.01 Angstrom
	if(T1.eq.0) T1 = 300
	if(T2.eq.0) T2 = 300	
	...
	if(T1.gt.T2) then
		...
		!enddo
                ...
                if(pos_hot_zone1.eq.pos_hot_zone2) then
                        rs = 6.2134*pos_hot_zone1 - 0.1d0; ls = 6.2134*pos_hot_zone1 - 1.1d0
		...
		write(20,*) igmol,part,cartesian,Ex_old,Ex_new
